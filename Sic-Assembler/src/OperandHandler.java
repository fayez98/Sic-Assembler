		    		...
		    		if(operand.charAt(0)=='@'||operand.charAt(0)=='#'){
		    			...
		    			try {
			    			...
			    			ins.appendErrorMessage("\t\t****Error: Problem with operand");
                                                System.out.println("Number Expcetion");
		    	        }
		    		}
}
public static void findErrors(Instruction ins){
    ...
    {   
        ...
        if(isTwoRegisters(ins)){
            System.out.println("Register2" +ins.mnemonic);
            twoRegisters(ins);
            System.out.println("first R"+ins.getR1()+"second"+ins.getR2()+"" );
        }
        else {
            System.out.println("Register1" +ins.mnemonic);
            singleRegister(ins);
        }
    }else{ // non register
        System.out.println("Memory" +ins.mnemonic);
        memoryErrors(ins);
